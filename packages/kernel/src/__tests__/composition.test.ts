...
describe('composition', () => {
  ...
  it('composing an organism inside another places it within the parent boundary', async () => {
    ...
    await composeOrganism(
      { parentId: album.id, childId: song.id, composedBy: userId },
      composeDeps(),
    );
  ...
  it('an organism can only have one parent (tree not DAG)', async () => {
    ...
    await composeOrganism(
      { parentId: album1.id, childId: song.id, composedBy: userId },
      composeDeps(),
    );

    await expect(
      composeOrganism(
        { parentId: album2.id, childId: song.id, composedBy: userId },
        composeDeps(),
      ),
  ...
  it('an organism cannot be composed inside itself', async () => {
    ...
    await expect(
      composeOrganism(
        { parentId: organism.id, childId: organism.id, composedBy: userId },
        composeDeps(),
      ),
    ).rejects.toThrow(CompositionError);
  });
